...
% Low pass filter (3 Hz) using FIR filter
...
% Take every 100th sample, creating a downsampled time vector
downsampled_time = time_vector(1:downsampling_factor:end);

% Downsample envelope by selecting every 100th data point 
% (in order to have fewer samples for plotting)
downsampled_emg = envelope_emg(1:downsampling_factor:end);

% Normalize downsampled envelope
norm_downsampled_emg = downsampled_emg./max(downsampled_emg);
...
subplot(3, 1, 2);
plot(downsampled_time, rectified_emg(1:downsampling_factor:end), 'g');
hold on;
plot(downsampled_time, downsampled_emg, 'k');
title('Rectified EMG and Envelope');
xlabel('Time (s)');
ylabel('Amplitude');
legend('Rectified EMG', 'Envelope');

% Subfigure 3: Movement Signal (Accelerometer) and Envelope
subplot(3, 1, 3);
...
hold on;
plot(downsampled_time, norm_downsampled_emg, 'k');
title('Movement Signal (X, Y, Z) and Normalized Envelope');
xlabel('Time (s)');
ylabel('Amplitude');
